...
char * sort_By_This_Value[2];       //this will store the input sorting column name must be provided
char * ouput_Place_Char;            //this will store the output directory name if provided
...
int main(int argc, char* argv[]) {
    ...
    sorting_Directory_Char = (char *) malloc (sizeof(char) * 3000);
...
//output_Place_Char = argv[4];
					...
					closedir(sorting_Directory_Char);
                    free(sorting_Directory_Char);
                    free(output_Place_Char);
                    if (movie_List.pRecArray != NULL) {
		              clearMovieList(&movie_List);
...
void* void_Of_Sort_The_List (void * arg){
    File * file_Temp = (File *) arg;
    int * temp = (int *) malloc (sizeof(int));
    * temp = sort_The_List(sort_By_This_Value, file_Temp, output_Place_Char);
    pthread_exit(temp);
}
...
//stuffing the header struct pointer with column names
            while (temp_xyz_x < no_Of_Columns_In_Header) {
                ...
                (struct_Pointer_Header + temp_xyz_x)->column_Name = (char *) malloc (sizeof(char) * strlen(word));
                strcpy((struct_Pointer_Header + temp_xyz_x)->column_Name, word_Two);
                temp_xyz_x++;
            }
...
//printf("CHECKING THIS FILE : %s with this column %s\n\n", name, column);
	...
	if((strlen(name) > (strlen(column) + 4)) && !(strcmp(name + strlen(name) - (strlen(column) + 4), test_Name))){
		fprintf(stderr, "%s - This file is already sorted by the sorted\n", name);
	...
	fprintf(stderr, "%s - This isn't a CSV file!\n", name);
	return -1;
}
...
void * scan_Directory(void * arg){
    ...
    char * sorting_Directory_Char_Path = (char *) arg;
...
//printf("This is the file name part one: %s\n", directory_Info->d_name);
		...
		*/
        sorting_Directory_Char_Path = strcat(sorting_Directory_Char_Path, "/");
        sorting_Directory_Char_Path = strcat(sorting_Directory_Char_Path, directory_Info->d_name);
        
        
        pthread_create(&thread_Checker, NULL, is_DIR_CSV, (void *)sorting_Directory_Char_Path);
        pthread_join(thread_Checker, (void *)return_Value_Of_Thread);
    
        
        if(*(return_Value_Of_Thread)==0){
            //this is a sub-directory
            pthread_create(&thread_Array[total_Threads]), NULL, scan_Directory, (void *)sorting_Directory_Char_Path);
            total_Threads++;
        }
        else if(*(return_Value_Of_Thread) == 1){
            pthread_create(&thread_Array[total_Threads]), NULL, sort_The_List, (void *)....);
            total_Threads++;
        }
