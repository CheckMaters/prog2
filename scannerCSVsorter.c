	...
	if(argc == 5) {
			...
			if(strcmp("-c", argv[1]) == 0) {
				if(strcmp("-d", argv[3]) == 0) {
					...
					return_Val_For_Main = scan_Directory(directory, sort_By_This_Value, argv[4], output_Place, &PID_Counter);
					closedir(directory);
			...
			if(strcmp("-c", argv[3]) == 0){
				if(strcmp("-d", argv[1]) == 0) {
					...
					char* sort_By_This_Value = argv[4];
					char * output_Place = argv[2];
	
					printf("Initial PID: %d\nPIDS of all child Processes: \n", parents_PID);

					return_Val_For_Main = scan_Directory(directory, sort_By_This_Value, argv[2], output_Place, &PID_Counter);
					closedir(directory);
					if(getpid() == parents_PID){
						printf("\nTotal Number of Processes: %d\n", PID_Counter);
					}
					...
					char* sort_By_This_Value = argv[4];
					printf("Initial PID: %d\nPIDS of all child Processes: \n", parents_PID);
					return_Val_For_Main = scan_Directory(directory, sort_By_This_Value, directory_Char, argv[2], &PID_Counter);
					closedir(directory);
					if(getpid() == parents_PID){
						printf("\nTotal Number of Processes: %d\n", PID_Counter);
					}
	...
	if(argc == 7) {
			if(strcmp("-c", argv[1]) == 0){
				if(strcmp("-d", argv[3])==0 && strcmp("-o", argv[5]) == 0){
						...
						DIR * output_Place = opendir(print_Here);
						if(output_Place == NULL){
							fprintf(stderr, "Error! Given output directory is pointing to NULL.\n");
							return -1;
						}
						...
						char print_Here[strlen(argv[4] + 1)];
						...
						return_Val_For_Main = scan_Directory(directory, sort_By_This_Value, argv[6], print_Here, &PID_Counter);
						closedir(directory);
						if(getpid() == parents_PID){
							printf("\nTotal Number of Processes: %d\n", PID_Counter);
						}
				...
				if(strcmp("-d", argv[1])==0 && strcmp("-o", argv[5]) == 0){
						...
						return_Val_For_Main = scan_Directory(directory, sort_By_This_Value, argv[2], print_Here, &PID_Counter);
						closedir(directory);
						if(getpid() == parents_PID){
							printf("\nTotal Number of Processes: %d\n", PID_Counter);
						}
				...
				if(strcmp("-d", argv[1])==0 && strcmp("-o", argv[3]) == 0){
						...
						char* sort_By_This_Value = argv[6];
