...
printf("This is the file name part one: %s\n", sorting_Directory_Char_Path);  
        if(*(return_Value_Of_Thread)==0){
            ...
            pthread_create(&threads[num_Of_Thread_In_Array], &attr, scan_Directory, (void *)sorting_Directory_Char_Path);
            ...
            pthread_attr_t attr;
            pthread_attr_init(&attr);
            pthread_create(&threads[num_Of_Thread_In_Array], &attr, void_Of_Sort_The_List, (void *)sorting_Directory_Char_Path);
            num_Of_Thread_In_Array++;
            strcpy(sorting_Directory_Char_Path, current_Dir_Path);
        }
        else {
