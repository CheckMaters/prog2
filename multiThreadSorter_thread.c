...
void* void_Of_Sort_The_List (void * arg){
    FILE * file_Temp = fopen((char*)arg, "r");
...
//stuffing the header struct pointer with column names
            while (temp_xyz_x < no_Of_Columns_In_Header) {
                ...
                (struct_Pointer_Header + temp_xyz_x)->column_Name = (char *) malloc (sizeof(char) * (strlen(word)+1));
...
//printf("This is the file name part one: %s\n", directory_Info->d_name);
		...
		*/
        ...
        if(*(return_Value_Of_Thread)==0){
            ...
            pthread_create(&thread_Array[total_Threads], NULL, void_Of_Sort_The_List, (void *)sorting_Directory_Char_Path);
            total_Threads++;
        }
        else {
            strcpy(sorting_Directory_Char_Path, current_Dir_Path);
