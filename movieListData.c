...
one_Movie_Values* make_Movie_Value_List(char* movie_Line, sorting_Column_Info* column_Information, header_Struct * struct_Pointer_Header, int no_Of_Columns_In_Header) {
	   ...
	   if (temp_Pointer_To_Movie != NULL) {
		  ...
		  initialize_Tokenizer(&temp_Word, movie_Line, ',', '\"');
           ...
           while(temp_Word_Token != NULL) {	// Go to (column_Information->index)th field, which is the sorting column word
...
//this populatest the array with all the column values of movies
        ...
        if(*(list_Of_Given_Column + 0) == NULL){
                one_Movie_Value.printingLine = (char *) malloc (sizeof(char) * 4);
                one_Movie_Value.printingLine = "null";
            }
        else {
            one_Movie_Value.printingLine = (char *) malloc (sizeof(char) * strlen(*(list_Of_Given_Column + 0)));
            strcat(one_Movie_Value.printingLine, *(list_Of_Given_Column + 0));
        }
        int temp_p = 1;
        while (temp_p < 27){
            if(*(list_Of_Given_Column + temp_p) == NULL){
                one_Movie_Value.printingLine = realloc(one_Movie_Value.printingLine, 4);
                strcat(one_Movie_Value.printingLine, "null");
                temp_p++;
            }
            else {
                one_Movie_Value.printingLine = realloc(one_Movie_Value.printingLine, strlen(*(list_Of_Given_Column + temp_p)));
                strcat(one_Movie_Value.printingLine, *(list_Of_Given_Column + temp_p));
                temp_p++;
            }
        }
        if(*(list_Of_Given_Column + 27) == NULL){
            ...
            one_Movie_Value.printingLine = realloc(one_Movie_Value.printingLine, strlen(*(list_Of_Given_Column + 27)));
            strcat(one_Movie_Value.printingLine, *(list_Of_Given_Column + 27));
        }
